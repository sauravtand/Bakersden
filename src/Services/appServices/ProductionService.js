import { GetChalanDetailsByDate, GetChalanItemDetailsByChalanId, GetProductionDetailsByDate, InsertChalanDetailsWithSeparateItemsDetails, InsertChalanWithItemDetails, InsertUpdateDayWiseProductionDetails, UpdateChalanItems, UpdateDeliveryChalan } from "../constants/url";
...
export const GetChalanDetailByDate = async (data, successCallback) => {
    try {
        const response = await fetch(`${GetChalanDetailsByDate}?fromdate=${data.fromdate}&todate=${data.todate}`)
        if (response?.status === 200) {
            successCallback(response?.data)
        } else {
            successCallback([])
...
export const GetChalanItemDetailsByChalansId = async (data, successCallback) => {
    try {
        const response = await fetch(`${GetChalanItemDetailsByChalanId}?chalanNo=${data}`)
        if (response?.status === 200) {
            successCallback(response?.data)
        } else {
            successCallback([])
